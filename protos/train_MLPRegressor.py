...
if __name__ == '__main__':
    ...
    handler = FileHandler(DIR + 'train_MLPRegressor.py.log', 'a')
    ...
    all_params = {
        ...
        'batch_size': [2 ** i for i in range(0, 3)],
    ...
    fe_gs = GridSearchCV(MLPRegressor(), all_params, scoring='neg_mean_squared_error', n_jobs=-1, cv=5, verbose=1)
    fe_gs.fit(X_train_fe, y_fe_train)
    clf_fe = MLPRegressor(**fe_gs.best_params_)
    clf_fe.fit(X_train_fe, y_fe_train)

    logger.info('formation_energy_ev_natom train end')

    bg_gs = GridSearchCV(MLPRegressor(), all_params, scoring='neg_mean_squared_error', n_jobs=-1, cv=5, verbose=1)
    bg_gs.fit(X_train_bg, y_bg_train)
    clf_bg = MLPRegressor(**bg_gs.best_params_)
    ...
    df_submit.to_csv(DIR + 'submit_MLPRegressor.csv', index=False)
